...
void ProcessGestureEvent(GestureEvent event)
{
    ...
    if (GESTURES.Touch.pointCount == 1)     // One touch point
    {
        ...
        else if (event.touchAction == TOUCH_ACTION_UP)
        {
            ...
            printf("GESTURE CURRENT: %d - (%f, %f)", GESTURES.current, event.position[0].x, event.position[0].y);
            ...
            printf("Drag distance [%f] (FORCE: %f %f)\n", GESTURES.Drag.distance, GESTURES.Drag.intensity, FORCE_TO_SWIPE);
            ...
            if ((GESTURES.Drag.intensity > FORCE_TO_SWIPE) && (GESTURES.current != GESTURE_DRAG))
            {
                ...
                printf("SWIPE ANGLE: %f -> (%f %f) -> (%f %f)\n", GESTURES.Drag.angle, GESTURES.Touch.downPositionA.x, GESTURES.Touch.downPositionA.y, GESTURES.Touch.upPosition.x, GESTURES.Touch.upPosition.y);
